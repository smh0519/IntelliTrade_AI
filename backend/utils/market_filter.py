...
def load_state() -> dict:
    ...
    return {
        ...
        "bull_signal_start": None,       # BULL 복귀 신호 시작일 (ISO date)
        "emergency_phase": 0,            # 0=미발동, 1=1차완료, 2=2차완료
        "emergency_first_exit_date": None,  # 1차 청산일 (ISO date)
    }
...
def update_regime() -> tuple[MarketRegime, MarketRegime, dict]:
    ...
    if raw == MarketRegime.BULL and prev_regime != MarketRegime.BULL:
        ...
        if confirmed >= BULL_RECOVERY_CONFIRM_DAYS:
            ...
            new_regime = prev_regime  # 확인 기간 미달 → 현재 국면 유지
            logger.info(
